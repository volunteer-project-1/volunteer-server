...
describe("updatePreference Test", () => {
  ...
  it("If success return updated Preference", async () => {
    const userId = 1;

    const spy = jest.spyOn(resumeService, "updatePreference");

    const updatedData: IUpdatePreference = {
      preference: { ...newPreferenceFactory() },
    };

    const mock = prismaMock.preferences.update.mockResolvedValue(
      updatedData as any
    );

    const result = await resumeService.updatePreference(userId, updatedData);
