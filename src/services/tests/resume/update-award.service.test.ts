...
import { updateAwardDTO } from "../../../types";
...
describe("updateAward Test", () => {
  ...
  it("If success return changedRows", async () => {
    ...
    const updatedData: updateAwardDTO = {
      award: { institute: "수정된 학회" },
    };

    const [result] = await resumeService.updateAward(insertId, updatedData);
