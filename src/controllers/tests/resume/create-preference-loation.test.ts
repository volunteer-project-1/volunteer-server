...
describe("create PreferenceLocation test", () => {
  ...
  it("GET '/',If Validate fail, return 400", async () => {
    ...
    const res = await request(await startApp())
      .post(`${URL}/${preferenceId}/preference-location`)
      .send({ test: "asdf" });
  ...
  it("GET '/',If No resume, return 404", async () => {
    ...
    const res = await request(await startApp())
      .post(`${URL}/${preferenceId}/preference-location`)
      .send(newPreferenceLocationFactory());
  ...
  it("GET '/',If Created, return 200", async () => {
    const userId = 1;
    const { preference } = await resumeService.createResume(
      userId,
      newResumeAllFactory()
    );
    const res = await request(await startApp())
      .post(`${URL}/${preference.id}/preference-location`)
      .send(newPreferenceLocationFactory());
