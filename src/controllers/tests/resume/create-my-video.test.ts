...
import { newMyVideoFactory, newResumeAllFactory } from "../../../factory";
...
describe("create MyVideo test", () => {
  ...
  it("GET '/',If Created, return 200", async () => {
    const userId = 1;
    const { resume, myVideo } = await resumeService.createResume(
      userId,
      newResumeAllFactory()
    );

    await resumeService.deleteMyVideo(myVideo.id);
